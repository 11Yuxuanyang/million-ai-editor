...
| 多个任务或代理需要显得正在同时运行 | 用六至九个真实界面组成不规则三维工作空间；保留一个清楚主界面，让镜头斜向穿过不同深度，界面内部持续运行。 | `technique.ui.parallel-interface-swarm` → `motion.composition.parallel-interface-swarm.v1` | [并行界面群](../../../references/asset-library/motion-recipes/parallel-interface-swarm.md)、[获批参考](../../../library/references/reference.ui.parallel-interface-swarm.v1/reference.yaml) |
